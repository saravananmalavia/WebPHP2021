
<ul style="list-style:none; padding:0px; line-height:24px;">
                    <li><a href="../login/login.php" style="color:#333;">Home</a></li>
                    <!-- <li><a href="#" style="color:#333;">About</a></li> -->
                    <li><a href="#" style="color:#333;">Student</a></li>
                    <li><a href="../course_details/course_view_all.php" style="color:#333;">Course</a></li>
                    <li><a href="../student_course/studentCourse_view_all.php" style="color:#333;">Student Enrollment</a></li>
                    <li><a href="../student_personal_details/student_personal_details_view_all.php" style="color:#333;">Student Details</a></li>
                    <li><a href="../exam_details/exam_View_All.php" style="color:#333;">Exam</a></li>
                    <li><a href="../staff_details/staff_View_All.php" style="color:#333;">Staff</a></li>
                    
                </ul>